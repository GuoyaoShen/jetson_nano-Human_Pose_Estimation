...
class TfPoseEstimator:
    ...
    # ===new added
    @staticmethod
    def caclu_centers(npimg, humans, imgcopy=False):
        if imgcopy:
            npimg = np.copy(npimg)
        image_h, image_w = npimg.shape[:2]
    ...
    @staticmethod
    def get_humanfacebox(npimg, humans, imgcopy=False):
        if imgcopy:
            npimg = np.copy(npimg)
        image_h, image_w = npimg.shape[:2]
        ...
        list_faceboxes = []
        for idx, human in enumerate(humans):
            # get the class of human
            # print('class of human:', type(human).__name__)  # Human

            # get face box of each human
            tuple_face_box = human.get_face_box(image_w, image_h, mode=0)
            list_faceboxes.append(tuple_face_box)

        print('list_faceboxes', list_faceboxes)
        return list_faceboxes
